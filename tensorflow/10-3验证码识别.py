...
BATCH_SIZE = 25
# tfrecord文件存放路径
TFRECORD_FILE = "E:/SEU/CodeCpp-learning/tensorflow/captcha/train.tfrecords"
...
with tf.Session() as sess:
    ...
    # 计算准确率
    correct_prediction_0 = tf.equal(tf.argmax(one_hot_labels0, 1), tf.argmax(logits0, 1))
    accuracy_0 = tf.reduce_mean(tf.cast(correct_prediction_0, tf.float32))
    correct_prediction_1 = tf.equal(tf.argmax(one_hot_labels1, 1), tf.argmax(logits1, 1))
    accuracy_1 = tf.reduce_mean(tf.cast(correct_prediction_1, tf.float32))
    correct_prediction_2 = tf.equal(tf.argmax(one_hot_labels2, 1), tf.argmax(logits2, 1))
    accuracy_2 = tf.reduce_mean(tf.cast(correct_prediction_2, tf.float32))
    correct_prediction_3 = tf.equal(tf.argmax(one_hot_labels3, 1), tf.argmax(logits3, 1))
    ...
    for i in range(6001):
        ...
        if i % 20 == 0:
            ...
            acc0, acc1, acc2, acc3, loss_ = sess.run([accuracy_0, accuracy_1, accuracy_2, accuracy_3, total_loss],
                                                     feed_dict={x: b_image,
                                                                y0: b_label0,
                                                                y1: b_label1,
                                                                y2: b_label2,
            ...
            print("Iter:%d  Loss:%.3f  Accuracy:%.2f,%.2f,%.2f,%.2f, Learning_rate:%.4f" % (i, loss_, acc0, acc1, acc2,
                                                                                            acc3, learning_rate))
